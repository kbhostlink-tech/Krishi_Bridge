...
function localizedUrl(locale: string, slug: string) {
  return `${SITE_URL}${locale === "en" ? "" : `/${locale}`}/${slug}`;
}

export function generateStaticParams() {
  return PUBLIC_INFO_PAGE_SLUGS.map((slug) => ({ slug }));
}
...
export default async function PublicInfoPage({
  ...
  return (
    <main className="bg-linen text-sage-900">
      ...
      <section className="bg-sand py-14 sm:py-18">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          ...
          <div className="mt-4 grid gap-8 lg:grid-cols-[1fr_0.35fr] lg:items-end">
            <div>
              <h1 className="max-w-4xl font-heading text-4xl font-semibold leading-tight text-sage-950 sm:text-5xl">
                {page.title}
              </h1>
      ...
      <section className="mx-auto grid max-w-7xl gap-8 px-4 py-12 sm:px-6 lg:grid-cols-[0.32fr_1fr] lg:px-8 lg:py-16">
          ...
          <div className="rounded-3xl border border-sage-100 bg-white p-5 shadow-sm">
            ...
            <ol className="mt-4 space-y-2 text-sm">
              {page.sections.map((section) => (
                <li key={section.id}>
                  <a href={`#${section.id}`} className="block rounded-2xl px-3 py-2 text-sage-600 transition-colors hover:bg-sage-50 hover:text-sage-950">
                    {section.title}
                  </a>
                </li>
              ))}
        ...
        <div className="space-y-5">
          {page.sections.map((section) => (
            <section key={section.id} id={section.id} className="scroll-mt-24 rounded-3xl border border-sage-100 bg-white p-6 shadow-sm sm:p-8">
      ...
      <section className="bg-sand py-12">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          <div className="flex flex-col gap-5 rounded-3xl border border-sage-100 bg-white p-6 shadow-sm sm:flex-row sm:items-center sm:justify-between sm:p-8">
            <div>
